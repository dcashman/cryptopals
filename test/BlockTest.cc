...
TEST(BlockTest, encryption_oracle) {
    ...
    BinaryBlob b1 = encryption_oracle(input);
    BinaryBlob b2 = encryption_oracle(input);
    EXPECT_NE(b1.hex(), b2.hex());
}

TEST(BlockTest, detect_oracle_mode) {
    // Let's make sure we are successful a bunch, so let's do 5 runs of ecb and
    // 5 of cbc, and pass if we get 100% on all.
    for (int i = 0; i < 5; i++) {
        EXPECT_TRUE(detect_oracle_mode(true /* cbc mode */));
    }
    for (int i = 0; i < 5; i++) {
        EXPECT_FALSE(detect_oracle_mode(false /* ecb mode */));
    }
}
